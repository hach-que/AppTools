	...
	for (int a = 1; a <= 10; a += 1)
	{
		...
		for (int i = 0; i <= FILESIZE; i += a * 10)
		{
			...
			if (res != 0)
			{
				...
				AppLib::Logging::showErrorO("inode %i to size %i",
					inodeid, i);
				AppLib::Logging::showErrorO("while increasing in blocks of %i.", a * 10);
		...
		for (int i = FILESIZE; i >= 0; i -= a * 10)
		{
			...
			if (res != 0)
			{
				...
				AppLib::Logging::showErrorO("inode %i to size %i.",
					inodeid, i);
				AppLib::Logging::showErrorO("while decreasing in blocks of %i.", a * 10);
				AppLib::Logging::showErrorO("  Result returned was: %s", getFSResultName(res));
				return 1;
			}
			if (!tc_verify(path, i))
