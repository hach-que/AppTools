...
void * appfs_init(struct fuse_conn_info *conn);
...
The function below replaces main() that is normally located
in FUSE-Ext2.

*/

void (*fuse_appfs_continue_execution)(void);

void * appfs_init(struct fuse_conn_info *conn)
{
	// initalize the filesystem
	void * ret = op_init(conn);
	fuse_appfs_continue_execution();
	return ret;
}

// Returns whether the FUSE Ext2 service could be started.
int extstart (char* disk_image, char* mount_point, int is_appfs, void (*appfs_ce_func)(void))
{
	fuse_appfs_continue_execution = appfs_ce_func;
        int err = 0;
        struct stat sbuf;
        struct fuse_args fargs = FUSE_ARGS_INIT(0, NULL);
        struct extfs_data opts;

	//printf("fuse-ext2: superblock offset: %i\n", SUPERBLOCK_OFFSET);
        debugf_main("version:'%s', fuse_version:'%d'", VERSION, fuse_version());

        memset(&opts, 0, sizeof(opts));

        // Set the options for FUSE-Ext2 manually since the user
        // is not going to be passing options here.
        opts.debug = 0;
        opts.silent = 0;
        opts.force = 0;
        opts.readonly = 0;
        opts.mnt_point = mount_point;
        opts.options = "";
        opts.device = disk_image;
        opts.volname = ""; // This would need to be set correctly
                           // if we were supporting FreeBSD.


        // Check that our disk image can be accessed.
        ...
        // Attempt to open the device among other things.
        if (do_probe(&opts) != 0) {
	...
	else
	{
        ...
        // Debug message for read-write mode.
        if (opts.readonly == 0) {
